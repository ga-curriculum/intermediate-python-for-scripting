...
## What are debugging and error handling?
...
The exception is raised when we call the `calculate_square_root` function with a negative number. We catch the exception using a `try`/`except` block and print the error message using the `str()` function to convert the exception object to a string.
...
In this example, we define a custom exception class called `NegativeNumberError` that inherits from the `Exception` class. We don't add functionality to the class, so we use the `pass` statement as a placeholder.
...
<div class="activity solo-exercise">
  <h2 class="title">Handle exceptions</h2>
