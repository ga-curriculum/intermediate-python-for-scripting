...
Two additional modes can be combined with the above modes. They allow you to read different types of files:
...
In this example, we open the file <code class="filepath">output.txt</code> in write mode using the `with` statement. We then use the `write()` method to write two lines of text to the file. The newline character (`\n`) is used to create a line break after the first line.
...
<div class="activity solo-exercise">
  <h2 class="title">Work with files</h2>
